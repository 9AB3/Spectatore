...
export default function PerformanceReview() {
  ...
  // helper to build daily + cumulative series for a given set of rows
  ...
  const dailyMax = useMemo(() => {
    let m = 0;
    for (const pt of seriesCurrent) {
      if (pt.value > m) m = pt.value;
    }
    for (const pt of seriesCrew) {
      if (pt.value > m) m = pt.value;
    }
    return m;
  ...
  const cumMax = useMemo(() => {
    let m = 0;
    for (const pt of seriesCurrent) {
      if (pt.cumulative > m) m = pt.cumulative;
    }
    for (const pt of seriesCrew) {
      if (pt.cumulative > m) m = pt.cumulative;
    }
  ...
  function buildCumPath(
    ...
    series.forEach((pt, i) => {
      const xCenter =
        chartPaddingLeft + i * (barWidth + gap) + barWidth / 2;
  ...
  const crewMatch = crew.find(
    (c) => c.id === Number(selectedCrewId || 0),
  );
                          ...
                          <table className="w-full text-sm mt-2 table-fixed">
                            ...
                            <tbody>
                              ...
                              <tr>
                                ...
                                <td className="py-1 pr-4 w-1/4 text-right">
                                  {Number.isFinite(
                                    safeAvg(allNoBolts, allNoBoltsCount),
                                  )
                                    ? safeAvg(
                                        allNoBolts,
                                        allNoBoltsCount,
                              ...
                              <tr>
                                ...
                                <td className="py-1 pr-4 w-1/4 text-right">
                                  {Number.isFinite(
                                    safeAvg(allHoles, allHolesCount),
                                  )
                                    ? safeAvg(
                                        allHoles,
                                        allHolesCount,
                ...
                <>
                  ...
                  <div className="flex flex-wrap gap-2 items-end">
                    ...
                    <div>
                      ...
                      >
                        {(subsByActivityGraph[graphActivity] || []).map(
                          (s) => (

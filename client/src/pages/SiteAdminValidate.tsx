...
function dayClass(st: DayStatus) {
  switch (st) {
    case 'green':
      return 'bg-emerald-500 text-white hover:bg-green-600';case 'red':
  ...
  function getGroupValue(activity: string, objOrValues: any) {
    ...
    const top =
      objOrValues && typeof objOrValues === 'object'
        ? (act === 'hauling' || act === 'loading' ? objOrValues.source : objOrValues.location)
        : null;
...
const totalRows = useMemo(() => {
  ...
  function aggHaul(payloads: any[]) {
    ...
    for (const p0 of payloads || []) {
      ...
      const trucks = n(v['No of trucks'] ?? v['No. of trucks'] ?? v['Trucks'] ?? v['No of Trucks']);
      const weightPer = n(v['Weight'] ?? v['weight']); // t/truck
  ...
  const fmtTW = (trucks: number, tonnes: number) =>
    `${Math.round(trucks)} (${Math.round(tonnes)})`;
  ...
  function prodDrillm(payloads: any[]) {
    ...
    for (const p0 of payloads || []) {
      ...
      const v: any = p.values && typeof p.values === 'object' ? p.values : {};
      sum += n(v['Metres Drilled']) + n(v['Cleanouts Drilled']) + n(v['Redrills']);
    }
    return sum;
  }

  function primaryStopeBogging(payloads: any[]) {
    let sum = 0;
    for (const p0 of payloads || []) {
      const p: any = p0 || {};
      ...
      const v: any = p.values && typeof p.values === 'object' ? p.values : {};
      sum += n(v['Stope to Truck']) + n(v['Stope to SP']);
    }
    return sum;
  }
  ...
  function gsDrillm(payloads: any[]) {
    ...
    for (const p0 of payloads || []) {
      ...
      const v: any = p.values && typeof p.values === 'object' ? p.values : {};
      const direct = n(v['GS Drillm']);
      if (direct > 0) {
        sum += direct;
      } else {
        // Fallback: bolts * bolt length (e.g. "2.4m")
        const bolts = n(v['No. of Bolts'] ?? v['No of Bolts'] ?? v['No. of bolts'] ?? v['No of bolts']);
        const bl = String(v['Bolt Length'] ?? '').toLowerCase();
        const blNum = n(bl.replace(/[^0-9.]/g, ''));
  ...
  function faceDrillm(payloads: any[]) {
    ...
    for (const p0 of payloads || []) {
      ...
      const v: any = p.values && typeof p.values === 'object' ? p.values : {};
      const direct = n(v['Face Drillm']);
      if (direct > 0) {
        sum += direct;
      } else {
        const holes = n(v['No of Holes'] ?? v['No. of Holes']);
        const cut = n(v['Cut Length']);
  ...
  const hoistSum = (payloads: any[]) => {
    ...
    for (const p0 of payloads || []) {
      ...
      const v: any = p.values && typeof p.values === 'object' ? p.values : {};
      out.oreT += n(v['Ore Tonnes']);
      out.wasteT += n(v['Waste Tonnes']);
    }
    return out;
  };
  ...
  rows.push(
    ...
    { activity: 'Hoisting', sub: 'Ore / Waste', metric: 'Waste', ds: hoDS.wasteT, ns: hoNS.wasteT, total: hoAll.wasteT },
  );

  // ---- Haulage (kept as-is) ----
  rows.push(
    { activity: 'Haulage', sub: 'Ore / Waste', metric: 'Ore', ds: fmtTW(hds.oreTrucks, hds.oreT), ns: fmtTW(hns.oreTrucks, hns.oreT), total: fmtTW(htotal.oreTrucks, htotal.oreT) },
    { activity: 'Haulage', sub: 'Ore / Waste', metric: 'Waste', ds: fmtTW(hds.wasteTrucks, hds.wasteT), ns: fmtTW(hns.wasteTrucks, hns.wasteT), total: fmtTW(htotal.wasteTrucks, htotal.wasteT) },

    { activity: 'Haulage', sub: 'Production / Development', metric: 'Production', ds: fmtTW(hds.prodTrucks, hds.prodT), ns: fmtTW(hns.prodTrucks, hns.prodT), total: fmtTW(htotal.prodTrucks, htotal.prodT) },
    { activity: 'Haulage', sub: 'Production / Development', metric: 'Development Ore', ds: fmtTW(hds.devOreTrucks, hds.devOreT), ns: fmtTW(hns.devOreTrucks, hns.devOreT), total: fmtTW(htotal.devOreTrucks, htotal.devOreT) },
    { activity: 'Haulage', sub: 'Production / Development', metric: 'Development Waste', ds: fmtTW(hds.devWasteTrucks, hds.devWasteT), ns: fmtTW(hns.devWasteTrucks, hns.devWasteT), total: fmtTW(htotal.devWasteTrucks, htotal.devWasteT) },
  );
  ...
  const loadSum = (payloads: any[]) => {
    ...
    for (const p0 of payloads || []) {
      ...
      if (sub === 'Production') {
        out.primStope += n(v['Stope to Truck']) + n(v['Stope to SP']);
        out.rehandleStope += n(v['Stope SP to Truck']) + n(v['Stope SP to SP']);
      } else if (sub === 'Development') {
        out.primDev += n(v['Heading to Truck']) + n(v['Heading to SP']);
        out.rehandleDev += n(v['Dev SP to SP']) + n(v['Dev SP to Truck']);
      }
    }
    return out;
  };
  ...
  rows.push(
    { activity: 'Loading', sub: 'Production', metric: 'Primary stope bogging', ds: ldDS.primStope, ns: ldNS.primStope, total: ldAll.primStope },
    { activity: 'Loading', sub: 'Production', metric: 'Rehandle stope bogging', ds: ldDS.rehandleStope, ns: ldNS.rehandleStope, total: ldAll.rehandleStope },
    { activity: 'Loading', sub: 'Development', metric: 'Primary development bogging', ds: ldDS.primDev, ns: ldNS.primDev, total: ldAll.primDev },
    { activity: 'Loading', sub: 'Development', metric: 'Rehandle development bogging', ds: ldDS.rehandleDev, ns: ldNS.rehandleDev, total: ldAll.rehandleDev },
  );

  // ---- Production ----
  rows.push(
    { activity: 'Production', sub: '', metric: 'Production Drillm', ds: prodDrillm(payloadsDS), ns: prodDrillm(payloadsNS), total: prodDrillm(payloadsAll) },
...
const liveObjByValidatedId = useMemo(() => {
  ...
  for (const va of validatedActs as any[]) {
    ...
    const vValues: any = vObj?.values && typeof vObj.values === 'object' ? vObj.values : {};
    const vActName = getActNameFromObj(va as any, vObj);
    const vSubName = getSubNameFromObj(va as any, vObj);
    const vGrp = getGroupValue(vActName, vValues);
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900">
      ...
      <div className="flex items-center justify-between">
        ...
        <button className="px-4 py-2 rounded-xl bg-slate-900 text-white shadow-sm hover:bg-slate-800" onClick={() => nav('/siteadmin')}>Back</button>
      </div>
      ...
      <div className="rounded-2xl bg-white border border-slate-200 shadow-sm p-4">
        ...
        <div className="flex flex-wrap gap-2 items-center justify-between">
          <div className="flex items-center gap-2">
            <button className="px-3 py-1 rounded-full bg-white border border-slate-200 shadow-sm hover:bg-slate-50" onClick={() => setYear((y) => y - 1)}>‹</button>
                ...
                <div className="grid grid-cols-7 gap-1">
                  {cells.map((d, idx) => {
                    ...
                    return (
                      <button
                        key={idx}
                        className={`h-6 rounded-md text-[11px] ${dayClass(st)} ${isSel ? 'ring-2 ring-slate-400' : ''}`}
        ...
        <div className="flex gap-4 text-xs mt-3 opacity-70">
          <div className="flex items-center gap-2"><span className="inline-block w-3 h-3 rounded bg-emerald-500" />Validated</div><div className="flex items-center gap-2"><span className="inline-block w-3 h-3 rounded bg-rose-500" />Unvalidated</div>
        </div>
      ...
      {selectedDate && (
        <div className="rounded-2xl bg-white border border-slate-200 shadow-sm p-4">
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="flex items-center gap-2">
              <button className="px-4 py-2 rounded-xl bg-sky-600 text-white shadow-sm hover:bg-sky-700 disabled:opacity-50" onClick={saveEdits} disabled={loadingDay} type="button">
                Save edits
              </button>
              <button className="px-4 py-2 rounded-xl bg-emerald-600 text-white shadow-sm hover:bg-emerald-700 disabled:opacity-50" onClick={validate} disabled={loadingDay} type="button">
                Validate shift
              </button>
            </div>
          </div>
                  ...
                  for (const a of acts) {
                    ...
                    out.push(
                      <tr key={`act|||${a}`} className="border-b border-slate-200 bg-slate-50">
                        <td className="p-2 font-semibold" colSpan={4}>{a}</td>
                      </tr>,
                    );
                    for (const r of rows) {
                      ...
                      if (r.sub && r.sub !== lastS) {
                        lastS = r.sub;
                        out.push(
                          <tr key={`${a}|||sub|||${r.sub}`} className="border-b border-slate-200 bg-slate-50/60">
                            <td className="p-2 pl-6 font-semibold" colSpan={4}>{r.sub}</td>
                          </tr>,
                        );
                      }
                      out.push(
                        <tr key={`${r.activity}|||${r.sub}|||${r.metric}`} className="border-b last:border-b-0">
                          <td className="p-2 pl-10">{r.metric}</td>
                          <td className="p-2 text-right whitespace-nowrap">{typeof r.ds === 'string' ? r.ds : Number(r.ds || 0).toLocaleString()}</td>
                          <td className="p-2 text-right whitespace-nowrap">{typeof r.ns === 'string' ? r.ns : Number(r.ns || 0).toLocaleString()}</td>
                          <td className="p-2 text-right whitespace-nowrap">{typeof r.total === 'string' ? r.total : Number(r.total || 0).toLocaleString()}</td>
                        </tr>,
                      );
                    }
                  }
  ...
  <div className="text-xs opacity-80 flex items-center gap-2">
    ...
    <span>Edited (changed from finalized)</span>
                ...
                {(() => {
                  ...
                  return (
                    <div className="space-y-4">
                      {/* Admin locations are loaded from server (admin_locations) and filtered per row */}

                      {actNames.map((actName) => {
                        const rowsForAct = byAct[actName] || [];

                        // Determine grouping label: Hauling/Loading group by Source, everything else by Location.
                        // Hoisting has no group column.
                        ...
                        for (const r of rowsForAct) {
                          ...
                          Object.keys(values || {}).forEach((k) => {
                            const kl = String(k).toLowerCase();
                            // group field is rendered in its own dedicated column
                            if (kl === 'location' || kl === 'source') return;
                            colSet.add(k);
                          });
                        }
                        // Column ordering for the bottom tables:
                        // Location/Source is rendered as the dedicated group column.
                        // Then order: Equipment, From, To, Material, then everything else A→Z.
                        const prio = (k: string) => {
                          const kl = String(k || '').toLowerCase();
                        ...
                        for (const r of rowsForAct) {
                          ...
                          const values: any =
                            objCur?.values && typeof objCur.values === 'object' ? objCur.values : {};
                                      ...
                                      <Fragment key={sub}>
                                        {sub ? (
                                          <tr className="bg-slate-50 border-b border-slate-200">
                                            <td className="p-2 font-bold" colSpan={colSpan}>{sub}</td>
                                          </tr>
                                        ) : null}
                                          ...
                                          return (
                                            <Fragment key={`${sub}|||${loc}`}>
                                              ...
                                              {rows.map((r) => {
                                                ...
                                                const liveValues: any =
                                                  liveObj?.values && typeof liveObj.values === 'object' ? liveObj.values : {};
                                                ...
                                                return (
                                                  <tr key={(r as any)?.id} className="border-b border-slate-200">
                                                    ...
                                                    {groupKey ? (
                                                      <td className={`p-2 min-w-[220px] ${changedCellTdClass(diffGroup)}`}>
                                                        ...
                                                        {(() => {
                                                          ...
                                                          const isInList = opts.includes(currentGrp);
                                                            ...
                                                            <div>
                                                              <select
                                                                ...
                                                                onChange={(e) => {
                                                                  ...
                                                                  setEditedActs((prev) => ({
                                                                    ...prev,
                                                                    [Number((r as any)?.id)]: setValuesKey(currentObj, groupKey, next),
                                                                  }));
                                                                }}
                                                              >
                                                                <option value="">-</option>
                                                    ...
                                                    {cols.map((c) => {
                                                      const hasLive =
                                                        !!liveObj && Object.prototype.hasOwnProperty.call(liveValues, c);
                                                      ...
                                                      // If this field is not part of the original form for this row (and hasn't ever existed),
                                                      // don't allow admins to add "new" irrelevant data to the task.
                                                      const applicable =
                                                      ...
                                                      return (
                                                        <td
                                                          key={c}
                                                          className={`p-2 min-w-[140px] ${!applicable ? 'bg-slate-100 opacity-60' : changedCellTdClass(diff)}`}
                                                        >
                                                          ...
                                                          {(() => {
                                                            ...
                                                            if (cn === 'Equipment') {
                                                              ...
                                                              const optsEq = (adminEquipRows || [])
                                                                .filter((e: any) => allowedTypes.size === 0 || allowedTypes.has(String(e.type || '')))
                                                              ...
                                                              return (
                                                                ...
                                                                >
                                                                  <option value="">-</option>
                                                                  {!inList && cur ? (
                                                                    <option value={String(cur)} disabled>{String(cur)} (legacy)</option>
                                                                  ) : null}
                                                                  {optsEq.map((n: string) => (
                                                                    <option key={n} value={n}>{n}</option>
                                                                  ))}
                                                            ...
                                                            if (cn === 'Material') {
                                                              ...
                                                              const normalized = curStr === 'ore' || curStr === 'waste' ? curStr : (curStr ? curStr : '');
                                                              return (
                                                                <select
                                                                  className="input w-full"

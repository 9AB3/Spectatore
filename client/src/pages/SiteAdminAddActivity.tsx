        ...
        <div className="rounded-2xl bg-white border border-slate-200 shadow-sm p-4">
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <div className="input bg-slate-50">{site}</div>
            </div>
            <div>
              <label className="block text-sm font-medium">Date</label>
              <div className="input bg-slate-50">{date || '-'}</div>
            </div>
            <div>
                      ...
                      <div className="space-y-2">
                        {haulLoads.map((l, i) => (
                          <div key={i} className="flex items-center gap-2">
                            ...
                            />
                            <button type="button" className="px-3 py-2 rounded-xl border border-slate-200 bg-white hover:bg-slate-50" onClick={() => setHaulLoads((prev) => prev.filter((_, xi) => xi !== i))}>
                              ✕
                            </button>
                          </div>
                        ))}

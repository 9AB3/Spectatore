...
export default function Activity() {
  ...
  async function finishTask() {
    ...
    if (!isDevRehabOrGS) {
      // All other activities behave as before
      ...
      // Keep only bolt groups that have at least one value
      const nonEmptyGroups = (boltInputs || []).filter((b) => {
        const hasLength =
          (b.length && b.length !== 'Other') ||
          (b.length === 'Other' && b.lengthOther);
      ...
      if (nonEmptyGroups.length === 0) {
        // No bolt details entered: save a single activity with base values only
